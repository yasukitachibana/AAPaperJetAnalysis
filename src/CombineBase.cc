...
std::shared_ptr<Histogram> CombineBase::CreateHist( std::string hist_name ){
  std::cout << "[CombineBase] 1D Load Histogram: " << hist_name << std::endl;
  return std::make_shared<Hist1D>(hist_name);
}
...
void CombineBase::LoadHist( double jpmin, double jpmax,
  ...
  for(int i_pthat_bin = 0; i_pthat_bin < nPtHatBin; i_pthat_bin++ ){
    ...
    auto hist_this_bin = CreateHist( hist_name );
    hist_this_bin->Init();
