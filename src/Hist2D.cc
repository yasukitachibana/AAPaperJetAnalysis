...
void Hist2D::InitHist(){
  ...
  std::string bin_type = SetXML::Instance()->GetElementText({"binType"});
  
  if( bin_type == "fix" || bin_type == "fixed" || bin_type == "f"){
    
    std::cout << "[Hist2D] Bin Type:"<< bin_type <<std::endl;
    
    nBinX = SetXML::Instance()->SetXML::Instance()->GetElementInt({"nBinX"});
    boundX = SetXML::Instance()->SetXML::Instance()->GetElementVectorDouble({"boundX","Item"});
    
    nBinY = SetXML::Instance()->SetXML::Instance()->GetElementInt({"nBinY"});
    boundY = SetXML::Instance()->SetXML::Instance()->GetElementVectorDouble({"boundY","Item"});
    ...
    std::cout << "[Hist2D] Bin Type, "<< bin_type << ", is not supported" << std::endl;
    exit(-1);
  }
...
void Hist2D::Print(std::string name, bool addHistname /* = true */ ){
  ...
  ofs << "# x_low, x_center, x_high, y_low, y_center, y_high, value, error, n_jet, n_event"<<std::endl;
